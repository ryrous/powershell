...
Foreach ($Server in $Servers) {
    ...
    $PhysicalMemory = Get-WmiObject CIM_PhysicalMemory -ComputerName $Server | Measure-Object -Property Capacity -Sum | ForEach-Object {[Math]::Round($_.sum / 1GB)}
    #Get Disk Information. The data will be shown in a table as GB, rounded to the nearest GB.
    $DiskInfo = Get-WmiObject -Class Win32_LogicalDisk -ComputerName $Server | Measure-Object -Sum Size | ForEach-Object {[Math]::Round($_.sum / 1GB)}
