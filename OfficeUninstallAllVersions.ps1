### Remove All Versions of Microsoft Office ###
$PatternProductID = '(?(?=^[^\.]+$)\S*|(?<=\.)\S*)'
$xmlFile = "$env:TEMP\config.xml"

$Installs = @()
$Installs += Get-ItemProperty "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*" -ErrorAction SilentlyContinue | Where-Object {$_.DisplayName -like "Microsoft Office*"} | Where-Object {$_.PSChildName -match '^(?!{).*'} 
$Installs += Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*" -ErrorAction SilentlyContinue | Where-Object {$_.DisplayName -like "Microsoft Office*"} | Where-Object {$_.PSChildName -match '^(?!{).*'} 
